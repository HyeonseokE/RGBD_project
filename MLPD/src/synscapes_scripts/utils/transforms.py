...
class Compose(object):
    ...
    def __call__(self, img, img_d=None, vis_box=None, lwir_box=None, pair=1):

        for t in self.transforms:    

            if self.args != None and t.__class__.__name__ in self.args.upaired_augmentation:
                img, img_d, vis_box, lwir_box, pair = t(img, img_d, vis_box, lwir_box, pair)
            else:
                img, img_d, vis_box, lwir_box = t(img, img_d, vis_box, lwir_box)
        
        return img, img_d, vis_box, lwir_box, pair
...
class Resize(object):
    ...
    def __init__(self, size, mode, interpolation=Image.BILINEAR):
        assert isinstance(size, int) or (isinstance(size, Iterable) and len(size) == 2)
        self.size = size
        self.interpolation = interpolation
        self.mode = mode

    def __call__(self, img, img_d=None, boxes_vis=None, boxes_lwir=None):
        """
        Args:
        ...
        img_d = F.resize(img_d, self.size, self.interpolation) if img_d is not None else None
        # if self.mode == "train":
        #     boxes_vis = F.box_resize(np.array(boxes_vis), self.size[0], self.size[1])
        #     boxes_lwir = F.box_resize(np.array(boxes_vis), self.size[0], self.size[1])
        # else: # test
        #     pass
        boxes_vis = F.box_resize(np.array(boxes_vis), self.size[0], self.size[1])
        boxes_lwir = F.box_resize(np.array(boxes_vis), self.size[0], self.size[1])

        return img, img_d, boxes_vis, boxes_lwir
...
class ColorJitter(object):
    ...
    def __call__(self, img, img_d=None, boxes_vis=None, boxes_lwir=None):
        ...
        if random.random() < 0.5:
            ...
            return transform(img, img_d, boxes_vis, boxes_lwir)[:-1]
