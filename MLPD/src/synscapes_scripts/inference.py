...
def run_inference(model_path: str, isVisualize:bool) -> Dict:
    ...
    if isVisualize:
        batch_size = 1
    test_dataset = SynscapesDataset(args, condition="test")
...
def run_inference_train(model_path: str, isVisualize:bool) -> Dict:
    """Load model and run inference
    ...
    args = config.args
    batch_size = config.test.batch_size * torch.cuda.device_count()
    train_dataset = SynscapesDataset(args, condition="train")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model-path', type=str, default='/home/hscho/workspace/ssd2.5d/MLPD-Multi-Label-Pedestrian-Detection/src/jobs/2024-07-05_14h54m_/checkpoint_ssd300.pth.tar039',
                         help='Pretrained model for evaluation.')
    ...
    # results = run_inference_train(arguments.model_path, isVisualize = False)
    results = run_inference(arguments.model_path, isVisualize=True)
