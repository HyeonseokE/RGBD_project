...
parser.add_argument("--wandb", action="store_true", help="wandb usage", default = False)
parser.add_argument("--exp_name", type = str, help="experiments name", default='debug')
parser.add_argument("--num_workers", type = int, help="# of workers", default=0)
...
def main():
    ...
    if parser.wandb:
        wandb.init(project="MLPD|synscapes", entity="chohs5133", name = parser.exp_name)
        wandb.run.log_code("./", include_fn=lambda path: path.endswith(".py") or path.endswith(".ipynb"))
        

    # Initialize model or load checkpoint 512x640 455x570 720x1440
    if checkpoint is None:
        model = SSD300(n_classes=args.n_classes, resolution=args.test.input_size)
    ...
    train_dataset = SynscapesDataset(args = args, condition='train')
    ...
    jobs_dir = os.path.join('jobs', args.exp_time + '_' + parser.exp_name)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        ###################### for debug ######################
        # train_loss = 0
        # model_checkpoint_path = utils.save_checkpoint(epoch, model, optimizer, train_loss, jobs_dir) # jobs/2024-07-29_10h28m_debug/checkpoint_ssd300.pth.tar047
        # isVisualize = False
        # test_during_train(model_path = model_checkpoint_path, result_dir = jobs_dir, num_workers = parser.num_workers, epoch = epoch, isVisualize = isVisualize)
        ...
        train_loss, loc_loss, cls_loss, depth_loss = train_epoch(model=model,
        ...
        log_dict['losses_sum'] = train_loss
        log_dict['losses_loc_sum'] = loc_loss
        log_dict['losses_cls_sum'] = cls_loss
        log_dict['losses_depth_sum'] = depth_loss
...
def train_epoch(model: SSD300,
    ...
    return losses_sum.avg, losses_loc_sum.val, losses_cls_sum.val, losses_depth_sum.val
