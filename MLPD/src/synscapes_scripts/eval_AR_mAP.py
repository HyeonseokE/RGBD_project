...
from datasets import SynscapesDataset
    ...
    with torch.no_grad():
        for i, blob in enumerate(tqdm(dataloader, desc='Evaluating')): # "desc" : 진행바 앞에 텍스트 출력
            ...
            if isVisualize == True:
                ...
                visualize_RGB_with_GT(transform=transform_, fnt = fnt_, result=results[image_id.item()], image=image_vis,image_id=image_id, mode=dataloader.dataset.mode, gt_box=boxes)
...
def visualize_RGB_with_GT(transform, fnt, result, image, image_id, mode, gt_box):
    import time
    tic = time.time()
    ...
    # draw gt bbox
    xyxy_scaler_np = np.array([[570, 455, 570, 455]], dtype=np.float32)
    gt_box = gt_box[0] * xyxy_scaler_np
    IoU_thr = 0.5
    ...
    for i, box in enumerate(gt_box):
        draw1.rectangle(box.tolist(), width=1, outline="blue")

    for i, info in enumerate(result):
        box = info[:4]
        bbox = [box[0], box[1], box[0]+box[2], box[1]+box[3]]
        overlap = find_jaccard_overlap(gt_box, torch.tensor([bbox]))
        if sum(overlap > IoU_thr).item() > 0:
            draw1.text((bbox[0] +(bbox[2]-bbox[0]),bbox[1]), f"IoU:{overlap.max().item():.2f}", font=fnt)

        score = info[4]
        depth = info[5]
        draw1.rectangle(bbox, width=2, outline='red')
        # draw1.text(((bbox[2])/2, (bbox[3])/2), f"{depth}", font=fnt, fill = 'green')
...
def run_inference(model_path: str, num_workers, isVisualize:bool) -> Dict:
    ...
    test_dataset = SynscapesDataset(args = args, condition="test")
...
def main_test():
    ...
    # result/checkpoint_ssd300_pth_TEST_det
    ...
    user_submission_file = save_results_potenit(results, result_filename)
    test_annotation_file = config.PATH.JSON_GT_FILE
    print(f'user_submission_file name(DT) : {user_submission_file}')
    print(f'test_annotation_file name(GT) : {test_annotation_file}')

    unannotated_id = []
    # gt annotation을 불러옴
    potenitGt = COCO(test_annotation_file, unannotated_id) 
    # detection result file로 부터 검출결과를 읽어드림. # potenitDt.anns
    ...
    # depth_eval.Depth_Average_Precision(GT=GT, results=results)
...
def test_during_train(model_path, result_dir, num_workers, epoch, isVisualize):
    ...
    # "/jobs/2024-07-23_03h02m_/checkpoint_ssd300.pth.tar020"
    ...
    result_filename = os.path.join(result_dir,  f'{model_path}_TEST_det') # jobs/2024-07-29_10h29m_debug/checkpoint_ssd300_pth_TEST_det

    # Run inference
    results = run_inference(original_model_path, num_workers, isVisualize=isVisualize)
    ...
    # gt annotation을 불러옴
    synscapesGt = COCO(test_annotation_file, unannotated_id) # 'MDPI_test_solution.json'
    # detection result file로 부터 검출결과를 읽어드림. # potenitDt.anns
    check = synscapesGt.getImgIds()
            
    synscapesDt = synscapesGt.loadRes(user_submission_file, unannotated_id) # 'jobs/2024-07-03_05h23m_/Epoch040_test_det.txt'
    imgIds = sorted(synscapesGt.getImgIds()) # 정렬된 image_id들
    coco_eval = COCOeval(synscapesGt, synscapesDt, 'bbox', result_dir, epoch)
...
if __name__ == '__main__':  
    ...
    parser.add_argument('--model_path', type=str, default='/home/hscho/workspace/src/MLPD/src/synscapes_scripts/jobs/2024-07-29_08h12m_depth:6|epoch:100/checkpoint_ssd300.pth.tar065',
