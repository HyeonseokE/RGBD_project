...
class VGGBase(nn.Module):
    ...
    def forward(self, image_vis, image_depth):
        """
        Forward propagation.
        ...
        out_depth = F.relu(self.conv1_1_bn_lwir(self.conv1_1_lwir(image_depth)))  
        out_depth = F.relu(self.conv1_2_bn_lwir(self.conv1_2_lwir(out_depth))) 
        out_depth = self.pool1_lwir(out_depth)  

        out_depth = F.relu(self.conv2_1_bn_lwir(self.conv2_1_lwir(out_depth)))
        out_depth = F.relu(self.conv2_2_bn_lwir(self.conv2_2_lwir(out_depth))) 
        out_depth = self.pool2_lwir(out_depth) 

        out_depth = F.relu(self.conv3_1_bn_lwir(self.conv3_1_lwir(out_depth))) 
        out_depth = F.relu(self.conv3_2_bn_lwir(self.conv3_2_lwir(out_depth))) 
        out_depth = F.relu(self.conv3_3_bn_lwir(self.conv3_3_lwir(out_depth))) 
        out_depth = self.pool3_lwir(out_depth)

        out_depth = F.relu(self.conv4_1_bn_lwir(self.conv4_1_lwir(out_depth))) 
        out_depth = F.relu(self.conv4_2_bn_lwir(self.conv4_2_lwir(out_depth))) 
        out_depth = F.relu(self.conv4_3_bn_lwir(self.conv4_3_lwir(out_depth))) 
        out_depth = self.pool4(out_depth)

        #########################################################################
        conv4_3_feats = torch.cat([out_vis, out_depth], dim=1) # (B,C,H,W) # channel_wise concat
        conv4_3_feats = F.relu(self.feat_1_bn(self.feat_1(conv4_3_feats))) # 3x3 conv

        #########################################################################

        # Rescale conv4_3 after L2 norm
        norm = conv4_3_feats.pow(2).sum(dim=1, keepdim=True).sqrt() # channel wise sum
        ...
        out_depth = F.relu(self.conv5_1_bn(self.conv5_1(out_depth))) 
        out_depth = F.relu(self.conv5_2_bn(self.conv5_2(out_depth))) 
        out_depth = F.relu(self.conv5_3_bn(self.conv5_3(out_depth))) 
        out_depth = self.pool5(out_depth)
        
        out_vis = F.relu(self.conv6_1_bn(self.conv6_1(out_vis))) 
        out_vis = F.relu(self.conv6_2(out_vis))
        out_depth = F.relu(self.conv6_1_bn(self.conv6_1(out_depth))) 
        out_depth = F.relu(self.conv6_2(out_depth))

        #########################################################################

        conv6_feats = torch.cat([out_vis, out_depth], dim=1)
        conv6_feats = F.relu(self.feat_2_bn(self.feat_2(conv6_feats))) # 3x3 conv

        #########################################################################

        out_vis = F.relu(self.conv7_1(out_vis))
        out_vis = F.relu(self.conv7_2_bn(self.conv7_2(out_vis))) 
        out_depth = F.relu(self.conv7_1(out_depth))
        out_depth = F.relu(self.conv7_2_bn(self.conv7_2(out_depth)))

        #########################################################################

        conv7_feats = torch.cat([out_vis, out_depth], dim=1)
        conv7_feats = F.relu(self.feat_3_bn(self.feat_3(conv7_feats)))# 3x3 conv

        ######################################################################### 

        out_vis = F.relu(self.conv8_1(out_vis))
        out_vis = F.relu(self.conv8_2(out_vis)) 
        out_depth = F.relu(self.conv8_1(out_depth))
        out_depth = F.relu(self.conv8_2(out_depth)) 

        #########################################################################

        conv8_feats = torch.cat([out_vis, out_depth], dim=1)
        conv8_feats = F.relu(self.feat_4_bn(self.feat_4(conv8_feats))) # 3x3 conv

        ######################################################################### 

        out_vis = F.relu(self.conv9_1(out_vis))
        out_vis = F.relu(self.conv9_2(out_vis))
        out_depth = F.relu(self.conv9_1(out_depth))
        out_depth = F.relu(self.conv9_2(out_depth))

        #########################################################################

        conv9_feats = torch.cat([out_vis, out_depth], dim=1)
        conv9_feats = F.relu(self.feat_5_bn(self.feat_5(conv9_feats))) # 3x3 conv
        ...
        out_depth = F.relu(self.conv10_1(out_depth)) 
        out_depth = F.relu(self.conv10_2(out_depth))

        #########################################################################

        conv10_feats = torch.cat([out_vis, out_depth], dim=1)
        conv10_feats = F.relu(self.feat_6_bn(self.feat_6(conv10_feats))) # 3x3 conv
...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        # depth prediction convolutions (predict depth in localization boxes)
        self.dep_conv4_3 = nn.Conv2d(512, n_boxes['conv4_3'] * 1, kernel_size=3, padding=1)
        self.dep_conv6 = nn.Conv2d(512, n_boxes['conv6'] * 1, kernel_size=3, padding=1)
        self.dep_conv7 = nn.Conv2d(512, n_boxes['conv6'] * 1, kernel_size=3, padding=1)
        self.dep_conv8 = nn.Conv2d(512, n_boxes['conv7'] * 1, kernel_size=3, padding=1)
        self.dep_conv9 = nn.Conv2d(512, n_boxes['conv8'] * 1, kernel_size=3, padding=1)
        self.dep_conv10 = nn.Conv2d(512, n_boxes['conv9'] * 1, kernel_size=3, padding=1)
    ...
    def forward(self, conv4_3_feats, conv6_feats, conv7_feats, conv8_feats, conv9_feats, conv10_feats):
        ...
        # Predict classes in localization boxes
        d_conv4_3 = self.dep_conv4_3(conv4_3_feats) 
        d_conv4_3 = d_conv4_3.permute(0, 2, 3, 1).contiguous()
        d_conv4_3 = d_conv4_3.view(batch_size, -1, 1)

        d_conv6 = self.dep_conv6(conv6_feats)
        d_conv6 = d_conv6.permute(0, 2, 3, 1).contiguous()
        d_conv6 = d_conv6.view(batch_size, -1, 1)

        d_conv7 = self.dep_conv7(conv7_feats)
        d_conv7 = d_conv7.permute(0, 2, 3, 1).contiguous()
        d_conv7 = d_conv7.view(batch_size, -1, 1)

        d_conv8 = self.dep_conv8(conv8_feats)
        d_conv8 = d_conv8.permute(0, 2, 3, 1).contiguous()
        d_conv8 = d_conv8.view(batch_size, -1, 1)

        d_conv9 = self.dep_conv9(conv9_feats)
        d_conv9 = d_conv9.permute(0, 2, 3, 1).contiguous()
        d_conv9 = d_conv9.view(batch_size, -1, 1)

        d_conv10 = self.dep_conv10(conv10_feats)
        d_conv10 = d_conv10.permute(0, 2, 3, 1).contiguous()
        d_conv10 = d_conv10.view(batch_size, -1, 1)
        ...
        classes_scores = torch.cat([c_conv4_3, c_conv6, c_conv7, c_conv8, c_conv9, c_conv10],
                                   dim=1)
        depths = torch.cat([d_conv4_3, d_conv6, d_conv7, d_conv8, d_conv9, d_conv10], dim=1)
        return locs, classes_scores, depths
...
class SSD300(nn.Module):
    ...
    def __init__(self, n_classes, resolution = None):
        super(SSD300, self).__init__()
        self.resolution = resolution = f'{resolution[0]}x{resolution[1]}'
        self.n_classes = n_classes
        self.base = VGGBase()
    ...
    def forward(self, image_vis, image_depth):
        ...
        conv4_3_feats, conv6_feats , conv7_feats, conv8_feats, conv9_feats, conv10_feats = self.base(image_vis, image_depth)

        # Run prediction convolutions (predict offsets w.r.t prior-boxes and classes in each resulting localization box)
        locs, classes_scores, depths = self.pred_convs(conv4_3_feats, conv6_feats, conv7_feats, conv8_feats, conv9_feats, conv10_feats)  # (N, 8732, 4), (N, 8732, n_classes)
    ...
    def create_prior_boxes(self):
        ...
        """
        if self.resolution == '512x640': # (h, w) 
        ...
        elif self.resolution == '455x570':
            fmap_dims = {'conv4_3': [72,57],
                        'conv6': [36,29],
                        'conv7': [18,15],
                        'conv8': [9,8],
                        'conv9': [9,8],
        ...
        elif self.resolution == '720x1440':
            fmap_dims = {'conv4_3': [180,90],
                        'conv6': [90,45],
                        'conv7': [45,23],
                        'conv8': [23,12],
                        'conv9': [23,12],
    ...
    def detect_objects(self, predicted_locs, predicted_scores, predicted_depths, min_score, max_overlap, top_k):
        """
        ...
        for i in range(batch_size):
            # potenit data set bbox format : (x_min, y_min, x_max, y_max)
            ...
            class_predicted_depth = predicted_depths[i][score_above_min_score]
            ...
            class_predicted_depth = class_predicted_depth[sort_ind]
            ...
            image_depths.append(class_predicted_depth[~suppress])
            ...
            if len(image_boxes) == 0:
                image_depths.append(torch.LongTensor([0]).to(device))
            ...
            image_depths= torch.cat(image_depths, dim=0)
            ...
            if n_objects > top_k:
                ...
                image_depths = image_depths[sort_ind][:top_k]
            ...
            all_images_depths.append(image_depths)
            all_images_boxes.append(image_boxes)
            all_images_labels.append(image_labels)
            all_images_scores.append(image_scores)
            all_images_bg_scores.append(image_bg_scores)
            
        return all_images_boxes, all_images_labels, all_images_scores, all_images_depths, all_images_bg_scores  # lists of length batch_size
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, predicted_depths, boxes, labels, depths):
        ...
        # # of priors -> [570, 455] = 41760 / [512, 640] = 41760
        # # of prediction -> [570, 455] = [12, 33804, 4] / [512, 640] = [12, 33804, 4]
        assert n_priors == predicted_locs.size(1) == predicted_scores.size(1)
        ...
        true_depths = torch.zeros((batch_size, n_priors, 1), dtype=torch.float, device=device)

        # For each image
        for i in range(batch_size):
            n_objects = boxes[i].size(0) # num of objects of i-th image
            overlap = find_jaccard_overlap(boxes[i],self.priors_xy) # calculate IoU of each boxes
            ...
            # Depths for each prior
            depth_for_each_prior = depths[i][object_for_each_prior]
            
            # Set priors whose overlaps with objects are less than the threshold to be background (no object)
            label_for_each_prior[overlap_for_each_prior < self.threshold] = 0 # allocate no_obeject label as zero(0)
            
            depth_for_each_prior[overlap_for_each_prior < self.threshold] = 0 # allocate no_obeject depth as zero(0)
            # Store
            true_classes[i] = label_for_each_prior
       
            true_depths[i] = depth_for_each_prior.unsqueeze(1)
            
            # Encode center-size object coordinates into the form we regressed predicted boxes to
            # 여기서 GT.bbox를 xy_to_cxcy한 뒤, cxcy기준 priors와 gcxgcy로 변경 => GT.bbox를 (gcx, gcy, gw, gh)형식으로 변경
        ...
        if true_depths[positive_priors].shape[0] == 0:
            d_regress_loss = torch.tensor([0], device=device)
        else:
            d_regress_loss = self.smooth_l1(predicted_depths[positive_priors], true_depths[positive_priors])  # (), scalar
        
        # CONFIDENCE LOSS
        n_positives = positive_priors.sum(dim=1)  # (N)
        n_hard_negatives = self.neg_pos_ratio * n_positives  # (N)
        ignore_index = torch.nonzero((true_classes.view(-1) == -1), as_tuple=False) # ignore class -> all background image ->label = -1은 학습에 포함시키지 않음.

        # pair_index = torch.nonzero((true_classes.view(-1) == 3), as_tuple=False)
        pair_index = []
        true_classes = (true_classes.view(-1)) # (-1 ~ 10) -> (0 ~ 11)
        ...
        if np.isnan(np.array((conf_loss + self.alpha * loc_loss + self.alpha * d_regress_loss).cpu().detach())):
            import pdb;pdb.set_trace()
        # TOTAL LOSS
        return loc_loss + conf_loss + d_regress_loss, conf_loss , loc_loss, d_regress_loss, n_positives
    
# def _to_one_hot(y, n_dims, dtype=torch.cuda.FloatTensor):
def _to_one_hot(y, n_dims, dtype=torch.FloatTensor):
    scatter_dim = len(y.size()) # 1
    # y_tensor = y.type(torch.cuda.LongTensor).view(*y.size(), -1) # (270432, 1)
    y_tensor = y.type(torch.LongTensor).view(*y.size(), -1) # (270432, 1)
    zeros = torch.zeros(*y.size(), n_dims).type(dtype) # torch.Size[270432, 11]
